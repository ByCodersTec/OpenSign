...
import { appName, smtpenable, smtpsecure, updateMailCount, debugLog } from '../../Utils.js';
...
async function sendMailProvider(req) {
  ...
  debugLog(
    '[PLACEHOLDER_DEBUG] sendMailv3 (email sending) start',
    JSON.stringify({
      extUserId: extUserId || null,
      smtpEnabled: !!smtpenable,
      hasRecipient: !!req.params.recipient,
    })
  );
  ...
  try {
    ...
    if (transporterSMTP) {
      ...
      if (!res.err) {
        ...
        debugLog(
          '[PLACEHOLDER_DEBUG] sendMailv3 (email sending) end via SMTP',
          JSON.stringify({ extUserId: extUserId || null, durationMs: Date.now() - startedAt })
        );
      ...
      if (mailgunApiKey) {
        ...
        if (res.status === 200) {
          ...
          debugLog(
            '[PLACEHOLDER_DEBUG] sendMailv3 (email sending) end via Mailgun',
            JSON.stringify({ extUserId: extUserId || null, durationMs: Date.now() - startedAt })
          );
          return { status: 'success' };
        }
      } else {
        debugLog('[PLACEHOLDER_DEBUG] sendMailv3: no SMTP transporter and no Mailgun API key configured');
        return { status: 'error' };
      }
    }
  } catch (err) {
